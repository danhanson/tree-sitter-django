...
- **`bindings/`** holds a binding per language, generated by `tree-sitter init` from the `bindings` and
  `metadata` sections of `tree-sitter.json`. See [Bindings](#bindings).
...
language = tree_sitter.Language(tree_sitter_django.language())
